...
초기 값이 1인 counter와 이 상태를 변경하는 setter함수를 반환하는 간단한 hook을 만든다.  그리고 Component에서 이 hook을 사용하여 단순히 카운터 값을 기록하는 console.log를 추가한다. 
...
const ReactX = (() => {
  let state;
  const useState = (initialValue) => {
		// 이 line은 state를 1로 설정한다. 이것은 우리가 원하는 것입니다. 그러나 hook가 호출될 때마다 설정됩니다. 이것은 우리가 원하는 것이 아니다.
    state = initialValue;
...
이러한 과정을 통해 이제는 꽤 명백하게 hooks의 호출 순서가 언제나 동일하게 보장되어야 하는지 알수 있을 것이다. 
